...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    /* Expose globally pour le bouton Réglages de la tab bar */
    window._portfolioOpenSettings = openSettings;

    /* ── 2.2b TOGGLE THÈME DANS LE SETTINGS PANEL ─────────── */
    if (settingsPanel) {
      ...
      themeSection.innerHTML = `
        ...
        <div class="settings-row">
          <button class="size-btn" data-set-theme="light">☀️ Clair</button>
          <button class="size-btn" data-set-theme="dark">🌙 Sombre</button>
        </div>
      `;
      const firstSection = settingsPanel.querySelector('.settings-section');
      if (firstSection) settingsPanel.insertBefore(themeSection, firstSection);
      else settingsPanel.appendChild(themeSection);

      themeSection.querySelectorAll('[data-set-theme]').forEach(btn => {
        btn.addEventListener('click', () => {
          const val = btn.dataset.setTheme;
          root.setAttribute('data-theme', val);
          localStorage.setItem('theme', val);
          syncUI(val, root.getAttribute('data-accent'),
                 root.getAttribute('data-size'), root.getAttribute('data-font'));
          springPop(btn);
        });
      });

      /* Sync l'état actif maintenant que les boutons existent */
    ...
    (function injectTabBar() {
      ...
      if (page === 'index.html' || page === '') return; /* pas sur la home iOS */
      if (document.getElementById('site-tabbar')) return;

      const active = href => page === href ? ' class="active"' : '';
      const tabs = [
        { href: 'about.html',       label: 'Profil',    icon: '<circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/>' },
        { href: 'missions.html',    label: 'Missions',  icon: '<rect x="3" y="4" width="18" height="16" rx="2"/><path d="M8 2v4M16 2v4M3 10h18"/>' },
        { href: 'imagix.html',      label: 'Imagix',    icon: '<rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/><path d="M7 8l5 3 5-3"/>' },
        { href: 'cv.html',          label: 'CV',        icon: '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/>' },
        { href: 'contact.html',     label: 'Contact',   icon: '<path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.55 1.18h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.69A16 16 0 0 0 15.31 16.1l1.88-1.88a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/>' },
      ];
      ...
      bar.innerHTML = tabs.map(t => `
        ...
        <button id="site-tabbar-settings" type="button" aria-label="Réglages">
      ...
      const stBtn = document.getElementById('site-tabbar-settings');
      if (stBtn) {
        stBtn.addEventListener('click', () => {
          if (window._portfolioOpenSettings) {
            springPop(stBtn);
            setTimeout(window._portfolioOpenSettings, 60);
          }
        });
      }
    })();
